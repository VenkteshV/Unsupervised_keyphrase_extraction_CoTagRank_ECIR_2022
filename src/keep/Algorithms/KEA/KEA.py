...
import pke
import string
from keep.utility import load_stop_words, CreateGroundTruthFile, CrossValidation, getlanguage, CreateKeywordsFolder, LoadFiles, ComputeDF, Convert2TrecEval
...
class KEA(object):
    ...
    def TrainingModel(self, nFolds = 1):
        ...
        # Training the Model
        if nFolds == 1:  # It means that it should create a model under the entire dataset. The purpose here is jsut to extract keywords (without having in mind any kind of evaluation)
            ...
            for f in (range(1, nFolds + 1)):
                self.__pathToCollectionOfDocs = self.__pathData + "/Datasets/" + self.__dataset_name + "/folds/fold" + str(f) + "/train"
                self.__pathToDFFile =  self.__pathData + "/Models/Supervised/dfs/" + self.__dataset_name + "/folds" + "/fold" + str(f) + "/df.tsv.gz"
                self.__pathToKEAFile = self.__pathData + "/Models/Supervised/" + self.__algorithmName + "/" + self.__dataset_name + "/folds" + "/fold" + str(f) + '/model.pickle'
                self.__pathToKeaModelsFolder = self.__pathData + "/Models/Supervised/" + self.__algorithmName + "/" + self.__dataset_name + "/folds" + "/fold" + str(f)

                # Track the status of the task
                print(f"\n--> Processing fold {f}/{nFolds}", end='')

                self.TrainingKEAModel(self.__pathToCollectionOfDocs, groundTruthFile, self.__lan, self.__normalization, self.__pathToDFFile, self.__pathToKEAFile, self.__pathToKeaModelsFolder)
    ...
    def TrainingKEAModel(self, pathToCollectionOfDocs, groundTruthFile, lang, normalization, pathToDFFile, pathToKEAFile,
        ...
        if os.path.exists(pathToKEAFile):
            ...
            print(f"KEA Model doesn't exists. Let's create here: {pathToCollectionOfDocs}. It may take a while.")
    ...
    def Convert2Trec_Eval(self, EvaluationStemming=False):
        Convert2TrecEval(self.__pathToDatasetName, EvaluationStemming, self.__outputPath, self.__keywordsPath, self.__dataset_name, self.__algorithmName)
