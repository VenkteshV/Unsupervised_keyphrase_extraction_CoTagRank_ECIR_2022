...
class EmbedRank:
    ...
    def mmr(self, text_emb, phrases, phrase_embs, beta=0.55, top_n=15, alias_threshold=0.8):
        ...
        valid_candidates_mask = ~np.all(phrase_embs == 0, axis=1)  # Only candidates which are not unknown.
        phrase_embs =  phrase_embs[valid_candidates_mask, :]
        phrases = np.array(phrases)
        phrases = phrases[valid_candidates_mask]
