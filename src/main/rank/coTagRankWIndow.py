...
class CoTagRankWindow:
    """Implementation of unsupervised `phrase` extraction method using USE and topic embeddings and our custom ranking algorithm with window based co-occurence to form edges, like existing methods. This method tries to
    ...
    def RankPhrases(self, text_emb, phrases, phrase_embs, top_n=10, alias_threshold=0.8, highlight = False):
        ...
        for i, (node1, pos_start,pos_end) in enumerate(top_phrases):
            for j in range(i + 1, min(i + window, len(top_phrases))):
                node2, pos_start,pos_end = top_phrases[j]
                if node1 != node2:
